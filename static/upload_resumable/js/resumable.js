...
"use strict";
  ...
  var Resumable = function(opts){
    ...
    $.defaults = {
      ...
      md5sum: 'md5sum',
      totalFileMd5sum: 'totalFileMd5sum',
    ...
    function ResumableChunk(resumableObj, fileObj, offset, callback){
      ...
      $.test = function(){  // add by qw for md5sum
        if (!$.fileObj.spark){
          $.fileObj.md5 = "";
          $.fileObj.spark = new SparkMD5.ArrayBuffer();
        }
        var func   = ($.fileObj.file.slice ? 'slice' : ($.fileObj.file.mozSlice ? 'mozSlice' : ($.fileObj.file.webkitSlice ? 'webkitSlice' : 'slice')));
        var bytes  = $.fileObj.file[func]($.startByte,$.endByte);
        var fileReader = new FileReader();
        fileReader.onload = function(e) {
          $.fileObj.spark.append(e.target.result);
          var current_chunk_num = $.offset + 1;
          var total_chunk_num = $.fileObj.chunks.length;
          if (current_chunk_num == total_chunk_num){
            $.fileObj.md5 = $.fileObj.spark.end();
          }
          $.org_test()
        };
      ...
      $.org_test = function(){
        ...
        params.push([parameterNamespace + $.getOpt('totalFileMd5sum'), encodeURIComponent( $.fileObj.md5)].join('='));
      ...
      $.send = function(){  // add by qw for md5sum
        $.md5sum = "";
        var func   = ($.fileObj.file.slice ? 'slice' : ($.fileObj.file.mozSlice ? 'mozSlice' : ($.fileObj.file.webkitSlice ? 'webkitSlice' : 'slice')));
        var bytes  = $.fileObj.file[func]($.startByte,$.endByte);
        var fileReader = new FileReader();
        fileReader.onload = function(e) {
          $.md5sum = SparkMD5.ArrayBuffer.hash(e.target.result);
          $.org_send()
        };
        fileReader.readAsArrayBuffer(bytes);
      };
      ...
      $.org_send = function(){
        ...
        query[$.getOpt('md5sum')] = $.md5sum;
        query[$.getOpt('totalFileMd5sum')] = $.fileObj.md5;
        // Mix in custom data
        var customQuery = $.getOpt('query');
        if(typeof customQuery == 'function') customQuery = customQuery($.fileObj, $);
        $h.each(customQuery, function(k,v){
